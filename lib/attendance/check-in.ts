...
/**
 * Check-in pipeline (spec §17, §26). Runs as the signed-in Admin under RLS:
 *
 *  1. caller is an active Admin        -> unauthorized
 *  2. payload is vckc:<token>           -> invalid_qr
 *  3. child exists for the token        -> unknown_qr
 *  4. child is active                   -> inactive_child
 *  5. guardian relationship exists      -> unknown_qr
 *  6. a session is open                 -> no_open_session
 *  7. not already checked in            -> already_checked_in
 *  8. insert with age-as-of-session-date + snapshots; the UNIQUE constraint is
 *     the final defense if two scans race (reported as already_checked_in)
 *
 * Never throws for business outcomes; only truly unexpected failures -> error.
